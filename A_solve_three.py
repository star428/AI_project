...
@software: pycharm
@file: A_solve_three.py
@time: 2020/10/27 0:59
@desc:为曼哈顿距离
"""
import random
import time
...
def hn(srcLayout):
    ...
    for i in range(9):
        if i is not a:
            sum += abs(length[i][0] - length[destLayout.index(srcLayout[i])][0])\
                + abs(length[i][1] - length[destLayout.index(srcLayout[i])][1])
